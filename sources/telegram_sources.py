...
Frank Media, vc.ru), те же новости придут дважды под разными адресами:
ссылкой на сайт и ссылкой на пост. Дедупликация в БД идёт по URL и такие пары
не склеивает. Поэтому здесь только каналы без уже подключённого сайта — так,
@vcnews и @forbesrussia сюда не попали: их содержимое приходит через RSS
vc.ru и forbes.ru.

Не всякий предложенный канал годится: @sale_caviar («кабачковая икра по
акции») по содержимому — паблик про футбольный троллинг, а не бизнес или
кадры, и сюда не включён.
"""
...
class TelegramBusinessParser(TelegramParser):
    ...
    CHANNELS = (
        ...
        "ftsec",            # 42 секунды — сводки мировых деловых новостей
        "collegi",          # Передали коллегам — маркетинг и рекламная индустрия
        "markettwits",      # MarketTwits — рынки акций и ценных бумаг
    )
...
class TelegramHrParser(TelegramParser):
    ...
    CHANNELS = (
        ...
        "rus_hr",           # Русский HR
        "theblueprintcareer",  # «Карьера» The Blueprint — переходы в глянце, редактуре, PR
        "itunion_pulse",    # Пульс ИТ-Профсоюза — трудовые права в ИТ
    )

...
def test_hr_business_filter_ignores_substring_false_positives():
    """
    Реальный случай: раньше фильтр строился отдельной «голой» регуляркой без
    учёта неоднозначности маркеров, и «стал» матчился как подстрока внутри
    «сталелитейным» — с любым словом делового контекста рядом это пропускало
    в отчёт новость про открытие ж/д платформы.
    """
    assert not is_hr_business_news(
        'Платформу "Серп и Молот" украсили сталелитейным ковшом. '
        'Название дано по металлургическому заводу.'
    )
    assert not is_hr_business_news(
        "Стриминговый сервис продлил сериал на третий сезон. "
        "Он составляет компанию гангстерам."
    )
